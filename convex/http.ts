...
// RFC 3834 marks automatic replies so software can avoid answering them, and
// this is the one place that matters here. The drill path was protected by
// one-graded-answer-per-drill; once an emailed QUESTION is answered too, a
// holiday auto-responder and this inbox can talk to each other all night, at
// the price of a model call per turn.
//
// Checked before anything is written or scheduled, so it covers the drill
// path as well. `emailAsk`'s hourly cap is the backstop if a responder is
// rude enough to omit all of these.
function isAutomatic(headers: unknown, subject: unknown): boolean {
  if (typeof subject === "string" && /^\s*(out of office|automatic reply|auto:|autoreply)/i.test(subject)) {
    return true;
  }
  if (!headers || typeof headers !== "object" || Array.isArray(headers)) return false;
  const lower: Record<string, string> = {};
  for (const [key, value] of Object.entries(headers as Record<string, unknown>)) {
    if (typeof value === "string") lower[key.toLowerCase()] = value.toLowerCase();
  }
  // auto-submitted: anything but "no" means generated by software.
  const submitted = lower["auto-submitted"];
  if (submitted !== undefined && submitted.trim() !== "no") return true;
  if (lower["x-autoreply"] || lower["x-autorespond"] || lower["list-id"]) return true;
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    if (isAutomatic(incoming.headers, incoming.subject)) {
      console.warn("inbound looks automatic, ignored");
      return new Response("automatic reply ignored", { status: 200 });
    }
